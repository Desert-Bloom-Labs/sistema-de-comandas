        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center space-x-3">
            ...
            >
              ...
              <span>PAPELERA ({counts.inTrash})</span>
            </button>
            {showTrash && trashOrders.length > 0 && (
              <button
                onClick={() => {
                  if (
                    confirm(
                      '¿Vaciar permanentemente la papelera? Esta acción es irreversible.'
                    )
                  ) {
          ...
          <div className="flex items-center justify-center h-64">
            <div className="text-center">
              ...
              <p className="text-gray-500 text-lg font-bold">
                {showTrash ? 'PAPELERA VACÍA' : 'SIN PEDIDOS'}
              </p>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                {displayOrders.map(order => (
                  ...
                  >
                      ...
                      <div className="space-y-2 mb-3">
                        {order.items.map((item, index) => (
                          ...
                          >
                            <div className="flex items-center space-x-2">
                              <span
                                className={`w-2 h-2 rounded-full ${item.status === 'cancelled' ? 'bg-red-500' : 'bg-green-500'}`}
                              ></span>
                              <span
                                className={`font-bold text-sm ${item.status === 'cancelled' ? 'line-through text-red-500' : 'text-gray-900'}`}
                              >
                          ...
                          {!showTrash && (
                            <button
                              onClick={e => {
                                e.stopPropagation();
                                if (
                                  confirm('¿Enviar este pedido a la papelera?')
                                ) {
                          ...
                          {showTrash && (
                            <button
                              onClick={e => {
                                ...
                                if (
                                  confirm(
                                    '¿Eliminar permanentemente este pedido?'
                                  )
                                ) {
                    ...
                    {/* Priority Indicator */}
                    {!showTrash &&
                      (getElapsedTime(order.created_at).includes('h') ||
                        parseInt(getElapsedTime(order.created_at)) > 20) && (
              ...
              {selectedOrder ? (
                            ...
                            ?.items.map((item, index) => (
                              ...
                              >
                                <div className="flex items-center space-x-2">
                                  ...
                                  ></span>
                                  <span
                                    className={`text-sm ${item.status === 'cancelled' ? 'line-through text-red-500' : 'text-gray-900'}`}
                                  >
                                    {item.quantity}x {item.product_name}
                                  </span>
                                </div>
                ...
                <div className="flex items-center justify-center h-full">
                  <div className="text-center text-gray-500">
                    ...
                    <p className="font-medium">Selecciona un pedido</p>
                    <p className="text-sm">para ver detalles</p>
                  </div>
                </div>
              )}
            </div>
