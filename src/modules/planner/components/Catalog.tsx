...
// Tipo para las mesas del restaurante
...
const Catalog: React.FC = () => {
  ...
  useEffect(() => {
    const loadRestaurantTables = async () => {
      ...
      try {
        const tables =
          await tablePlannerIntegrationService.getAvailableTablesForAssociation();
    ...
    // Recargar cada 5 segundos para ver nuevas mesas
    const interval = setInterval(loadRestaurantTables, 5000);
    return () => clearInterval(interval);
  }, []);

  // Filtrar objetos por categoría y búsqueda
  const filteredItems = React.useMemo(() => {
    // Si estamos en la pestaña "Mesas del Restaurante", devolver las mesas
    ...
    // De lo contrario, filtrar objetos normales del catálogo
    let items =
  ...
  const handleItemClick = (item: CatalogItem | RestaurantTable) => {
    // Si es una mesa del restaurante, crear un item especial
    if (selectedCategory === 'restaurant-tables') {
      const table = item as RestaurantTable;
      const tableItem: CatalogItem = {
        id: `table-${table.id}`,
        type: 'table',
        name: `Mesa ${table.number} - ${table.name}`,
        icon: '🍽️',
        size: { width: 1, height: 1, depth: 1 }, // Mismo tamaño que mesa con sillas
        color: '#8B4513',
        category: 'Mesas del Restaurante',
        description: `${table.capacity} puestos - ${table.status}`,
        metadata: {
          tableId: table.id,
    ...
    <div className="w-80 bg-white border-r border-gray-200 p-4 space-y-4">
      <div>
        ...
        <div className="flex flex-wrap gap-2 mb-4">
          ...
          {/* Pestaña de Mesas del Restaurante */}
          <button
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center space-x-3">
                    <div className="text-2xl">
                      {selectedCategory === 'restaurant-tables'
                        ? '🍽️'
                        : (item as CatalogItem).icon}
                    </div>
                    <div>
                      <h4 className="font-medium text-gray-900 text-sm">
                        {selectedCategory === 'restaurant-tables'
                          ? `Mesa ${(item as RestaurantTable).number} - ${item.name}`
                          : item.name}
                      </h4>
                      {selectedCategory === 'restaurant-tables' ? (
                        <p className="text-xs text-gray-500">
                          {(item as RestaurantTable).capacity} puestos -{' '}
                      ...
                      <div className="flex items-center space-x-2 mt-1">
                        {selectedCategory === 'restaurant-tables' ? (
                          <>
                            ...
                            <span
                              className={`text-xs px-2 py-1 rounded font-medium ${
                                (item as RestaurantTable).status === 'free'
                          ...
                          <>
                            <span className="text-xs bg-gray-100 px-2 py-1 rounded">
                              {(item as CatalogItem).size.width}×
                              {(item as CatalogItem).size.depth}
                            </span>
                            <span className="text-xs bg-gray-100 px-2 py-1 rounded">
        ...
        {filteredItems.length > 0 && (
          <div className="pt-4 border-t border-gray-200">
            <div className="flex justify-between text-sm text-gray-600">
              <span>{filteredItems.length} objeto(s) encontrado(s)</span>
            </div>
          </div>
        )}
      </div>
