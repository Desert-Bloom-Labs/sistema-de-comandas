...
import {
  ...
  tablePlannerIntegrationService,
  type TablePlannerItem,
} from '../services/tablePlannerIntegration';
...
const createTableTexture = (
  ...
  try {
    ...
    if (!ctx) {
      console.error(
        'Imposible crear el contexto 2D para la textura de la mesa'
      );
      return null;
    }
...
const TableInfoDisplay3D: React.FC<{
  tableId: string;
  tableNumber?: number;
}> = ({ tableId, tableNumber }) => {
  ...
  useEffect(() => {
    const loadTableInfo = async () => {
      try {
        const tables =
          await tablePlannerIntegrationService.getAllTablesWithStatus();
...
const RestaurantObject: React.FC<{
  ...
  const handlePointerDown = (e: ThreeEvent<PointerEvent>) => {
    ...
    if (
      usePlannerStore.getState().isPlacing ||
      usePlannerStore.getState().isExpandingLine
    ) {
      return;
    }
    ...
    return (
      ...
      >
        ...
        {Array.from({ length: Math.max(2, Math.floor(length / 2)) }, (_, i) => {
          const footSpacing =
            length / (Math.max(2, Math.floor(length / 2)) + 1);
          ...
          return (
            <React.Fragment key={i}>
              <mesh
                position={
                  isHorizontal ? [footPos, -0.3, 0.2] : [0.2, -0.3, footPos]
                }
              >
        ...
        {Array.from({ length: Math.floor(length * 2) }, (_, i) => {
          ...
          return (
            <React.Fragment key={i}>
              ...
              <mesh
                position={
                  isHorizontal ? [chairPos, -0.15, 0.4] : [0.4, -0.15, chairPos]
                }
              >
              ...
              <mesh
                position={
                  isHorizontal ? [chairPos, 0.05, 0.4] : [0.4, 0.05, chairPos]
                }
              >
        ...
        {isSelected && (
          <mesh position={[0, item.size.height / 2 + 0.1, 0]}>
            <boxGeometry
              args={[item.size.width + 0.2, 0.1, item.size.depth + 0.2]}
            />
      ...
      >
        ...
        <group position={[0.25, 0.3, 0.25]}>
          ...
          <mesh position={[0, 0, 0]}>
            ...
            <meshStandardMaterial
              map={createFloatingNumber3D(
                liveTableData?.number || item.metadata?.tableNumber || 1
              )}
              transparent
              opacity={0.9}
...
const PlacingObject: React.FC = () => {
  ...
  const placementTexture = useMemo(() => {
    ...
    placingItem?.metadata?.tableId,
    placingItem?.metadata?.tableNumber,
    placingItem?.metadata?.capacity,
  ]);
  ...
  if (placingItem.metadata?.isLongTable) {
    ...
    const length = isHorizontal
      ? placingItem.size.width
      : placingItem.size.depth;
  ...
  // Renderizado especial para mesas de restaurante
  if (
    placingItem.metadata?.isRestaurantTable ||
    placingItem.id.includes('table-')
  ) {
    return (
      <group
        position={[
      ...
      >
        ...
        <mesh>
          ...
          <meshStandardMaterial
            color={
              placementTexture ? '#ffffff' : canPlace ? '#8B4513' : '#ef4444'
            }
        ...
        <group position={[0.25, 0.3, 0.25]}>
          ...
          <mesh position={[0, 0, 0]}>
            ...
            <meshStandardMaterial
              map={createFloatingNumber3D(
                placingItem.metadata?.tableNumber || 1
              )}
              transparent
              opacity={0.7}
              side={THREE.DoubleSide}
        ...
        <mesh position={[0, -placingItem.size.height / 2 - 0.01, 0]}>
          <boxGeometry
            args={[placingItem.size.width, 0.02, placingItem.size.depth]}
          />
          <meshStandardMaterial
            color={canPlace ? '#22c55e' : '#ef4444'}
            transparent
...
const Scene: React.FC = () => {
  ...
  useEffect(() => {
    if (currentLayout?.id) {
      tablePlannerIntegrationService
        .getPlannerItemsWithTableStatus(currentLayout.id)
        .then(setTableData)
        .catch(console.error);
    }
  }, [currentLayout?.id]);
  ...
  return (
    <>
      ...
      {currentLayout.items.map(item => {
        const itemTableData = tableData.find(
          td => td.planner_item.id === item.id
        );
        return (
          <RestaurantObject
            key={item.id}
...
const View3D: React.FC = () => {
  ...
  const handleMouseMove = (event: React.MouseEvent) => {
    if (isExpandingLine) {
      ...
      isPlacing &&
      usePlannerStore.getState().placingItem &&
      canvasRef.current
    ) {
    ...
    <div className="w-full h-full relative">
        ...
        <div className="absolute top-4 right-4 bg-white rounded-lg p-3 shadow-lg z-50 border">
          <div className="flex flex-col space-y-2">
            ...
            {selectedItem.metadata?.isRestaurantTable && (
              <TableInfoDisplay3D
                tableId={selectedItem.metadata.tableId}
                tableNumber={selectedItem.metadata.tableNumber}
              />
            )}
      ...
      {(isPlacing || isExpandingLine) && (
        <div className="absolute bottom-4 left-1/2 transform -translate-x-1/2 bg-blue-500 text-white px-4 py-2 rounded-lg shadow-lg z-10">
          {isPlacing
            ? "Cliquez pour placer l'objet"
            : 'Cliquez pour valider la ligne'}
          <br />
