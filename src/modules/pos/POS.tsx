...
export default function POS() {
  ...
  const cartTotal = selectedTable
    ...
    : cartHook.localCart.total_amount;
  ...
  const getCartTaxBreakdown = () => {
    ...
    const taxGroups = new Map<
      string,
      { rate: number; name: string; amount: number }
    >();
  ...
  const addToCart = (product: ProductDisplay) => {
    ...
    setShowSuccessMessage('Producto agregado al carrito');
    setTimeout(() => setShowSuccessMessage(null), 2000);
  };

  const removeFromCart = (productId: string) => {
    cartHook.removeFromCart(productId);
    setShowSuccessMessage('Producto eliminado del carrito');
    setTimeout(() => setShowSuccessMessage(null), 2000);
  };
  ...
  const clearCart = async () => {
    ...
    setShowSuccessMessage('Carrito limpiado');
    setTimeout(() => setShowSuccessMessage(null), 2000);
  };
  ...
  const handlePayment = async () => {
    if (!cartItems || cartItems.length === 0) {
      setShowErrorMessage('El carrito está vacío');
    ...
    try {
      ...
      const paymentMethodName =
        paymentMethods.find(m => m.id === paymentMethod)?.name || 'Desconocido';
      ...
      await paymentHook.processPayment(
        ...
        () => {
          ...
          setShowSuccessMessage('¡Pago completado exitosamente!');
      ...
      setShowErrorMessage(
        `Error de pago: ${error instanceof Error ? error.message : 'Error desconocido'}`
      );
  ...
  const sendToKitchen = async () => {
    try {
      ...
      setShowSuccessMessage('¡Pedido enviado a cocina exitosamente!');
      setTimeout(() => setShowSuccessMessage(null), 3000);
    } catch (error) {
      setShowErrorMessage('Error al enviar a cocina');
      setTimeout(() => setShowErrorMessage(null), 3000);
    }
  };
  ...
  const filteredProducts = enrichedProducts?.filter(product => {
    ...
    const matchesSearch = (product as any).name
      .toLowerCase()
