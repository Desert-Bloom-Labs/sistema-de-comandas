...
export default function ModernTablesSection({
  ...
  const [reservations, setReservations] = useState<Map<string, string>>(
    new Map()
  );
    ...
    const fetchReservations = async () => {
      try {
        ...
        const todayReservations =
          await reservationService.getReservationsWithTableInfo(today);

        // Crear un mapa table_id -> hora de reserva
      ...
      <div className="flex-1 overflow-x-auto overflow-y-hidden p-3 min-h-0 scrollbar-hide">
        {tables.length === 0 ? (
          <div className="flex flex-col items-center justify-center text-slate-400 h-full">
            ...
            <p className="text-xs text-slate-500">No hay mesas configuradas</p>
          </div>
        ) : (
