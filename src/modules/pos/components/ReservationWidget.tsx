...
export default function ReservationWidget({
  ...
  const getTableNumber = () => {
    // Prioridad 1: table_number (ahora disponible gracias a getReservationsWithTableInfo)
    if (reservation.table_number) {
      return reservation.table_number;
    }
    // Prioridad 2: Intentar extraer el número del ID
  ...
  const getStatusText = () => {
    ...
    if (reservation.minutesUntil < 15) {
      return `LLEGANDO EN ${reservation.minutesUntil}min`;
    }
    ...
    return `RESERVADO PARA ${reservation.reservation_time}`;
  };
    ...
    return (
      <div className={`rounded-lg border-2 p-3 mb-3 ${getPriorityColor()}`}>
        ...
        <div className="mt-2 space-y-1">
          <div className="flex items-center space-x-2">
            <span className="text-sm font-medium">Mesa {getTableNumber()}</span>
  ...
  return (
    <div
      className={`rounded-xl border-2 p-4 mb-4 ${getPriorityColor()} relative`}
    >
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-3">
            ...
            <div className="flex items-center space-x-1">
              ...
              <span className="text-sm">{reservation.party_size} personas</span>
            </div>
          </div>
        </div>
