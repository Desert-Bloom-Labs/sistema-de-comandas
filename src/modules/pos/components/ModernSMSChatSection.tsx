...
export default function ModernSMSChatSection({
  ...
  const loadTimelineEvents = async () => {
    ...
    try {
      const events =
        await timelineService.getFilteredTimelineEvents(timelineFilters);
  ...
  const generateTicketMessage = () => {
    ...
    if (currentOrder?.id) {
      ticketMessage += `Pedido #${currentOrder.id}\n`;
    }

    if (selectedTable) {
      ticketMessage += `Mesa #${selectedTable.id}${selectedTable.name ? ` (${selectedTable.name})` : ''}\n`;
    }
    ...
    if (currentOrder?.items && currentOrder.items.length > 0) {
      ...
      ticketMessage += `• No hay artículos en el pedido\n`;
    }
      ...
      <div className="px-3 py-2 border-b border-slate-700 bg-slate-800 flex-shrink-0">
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-2">
            ...
            <h2 className="text-sm font-semibold text-white">Ticket SMS</h2>
            {selectedTable && (
        ...
        <div className="flex gap-2 mt-2">
          ...
          <select
            ...
            onChange={e =>
              setTimelineFilters(prev => ({
                ...prev,
                timeFilter: e.target.value as any,
              }))
            }
            className="flex-1 px-2 py-1 text-xs bg-slate-700 border border-slate-600 rounded text-white"
          >
            ...
            <option value="next2hours">±2 Horas</option>
            <option value="next4hours">±4 Horas</option>
          </select>
        </div>
      </div>
        ...
        <>
                          ...
                          <div className="bg-slate-50 rounded-xl p-4 max-w-[85%] border border-slate-200/60">
                            <div className="flex items-center justify-between mb-2">
                              ...
                              <span className="text-xs text-slate-400">
                                {timelineService.formatEventTime(
                                  event.timestamp
                                )}
                              </span>
                            </div>
          ...
          <div className="p-3 border-t border-slate-700 bg-slate-800 space-y-2 flex-shrink-0">
            ...
            <div className="flex gap-1 flex-shrink-0">
              ...
              >
                <RefreshCw
                  className={`w-4 h-4 ${isLoadingMessages || isLoadingTimeline ? 'animate-spin' : ''}`}
                />
              </button>
              <button
                ...
                title={isSendingSMS ? 'Enviando...' : 'Enviar SMS'}
              >
