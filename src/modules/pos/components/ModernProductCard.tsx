...
export default function ModernProductCard({
  ...
  const detectScriptType = (
    text: string
  ): 'latin' | 'chinese' | 'hindi' | 'arabic' | 'mixed' => {
    const chineseRegex =
      /[\u4e00-\u9fff\u3400-\u4dbf\u20000-\u2a6df\u2a700-\u2b73f\u2b740-\u2b81f\u2b820-\u2ceaf\uf900-\ufaff\u3300-\u33ff\u2e80-\u2eff\u2f00-\u2fdf\u31c0-\u31ef\u3200-\u32ff\u3400-\u4dbf\u4e00-\u9fff\uf900-\ufaff\u3300-\u33ff]/;
  ...
  return (
    <div
      className={`relative bg-gradient-to-br from-slate-700 to-slate-800 rounded-xl border-2 border-slate-600 hover:border-emerald-400 transition-all duration-300 cursor-pointer transform hover:scale-105 shadow-lg hover:shadow-emerald-500/20 ${
    ...
    >
      ...
      <div className="absolute top-1 left-1 z-10">
        <div
          className={`w-3 h-3 rounded-full flex items-center justify-center text-xs font-bold shadow-lg ${
            isOutOfStock
      ...
      <div className="relative h-full flex flex-col p-1.5 pt-3">
        ...
        <div className="flex-1 flex items-center justify-center text-center px-0.5">
          <h3
            className={`font-bold text-white leading-tight drop-shadow-sm text-center ${getAdaptiveFontSize(product.name)} ${getAdaptiveLineClamp(product.name)}`}
          >
            {product.name}
          </h3>
        </div>
