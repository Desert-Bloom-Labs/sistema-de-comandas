...
export default function SMSGatewayAndroidForm({
    ...
    try {
      ...
      if (result) {
        ...
        setTestResult({
          success: false,
          message:
            "Échec de l'envoi du SMS. Vérifiez la configuration et que le serveur SMS Gateway est démarré.",
        });
      }
    } catch (error) {
  ...
  const isFormValid =
    formData.device_ip && formData.username && formData.password;
      ...
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          ...
          <p className="text-sm text-gray-600">
            Configure your Android device with SMS Gateway app for local SMS
            sending
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div>
          ...
          <input
            ...
            value={formData.port}
            onChange={e =>
              handleInputChange('port', parseInt(e.target.value) || 8080)
            }
            placeholder="8080"
            min="1024"
            max="65535"
            className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
          />
          <p className="text-xs text-gray-500 mt-1">Port number (1024-65535)</p>
        </div>
            ...
            <div className="flex-1">
              ...
              {testResult.details && (
                <div className="mt-3 text-xs text-gray-600">
                  <p>
                    <strong>Numéro saisi:</strong>{' '}
                    {testResult.details.original_phone}
                  </p>
                  <p>
                    <strong>Numéro envoyé:</strong> {testResult.details.phone}
                  </p>
                  <p>
                    <strong>Message:</strong> {testResult.details.message}
                  </p>
                  <p>
                    <strong>Serveur:</strong> {testResult.details.device_ip}:
