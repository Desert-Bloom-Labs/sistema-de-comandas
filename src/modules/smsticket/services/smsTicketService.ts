...
export const smsTicketService = {
  ...
  async sendSMS(request: {
    ...
    try {
      console.log(
        'smsTicketService.sendSMS - Calling Tauri command with:',
        request
      );
  ...
  async getProviderSelection(): Promise<ProviderSelection> {
    try {
      ...
      return {
        default_provider: toFrontendProvider(
          (selection as any).default_provider as any
        ) as any,
  ...
  async updateProviderSelection(
    ...
    try {
      const updatedSelection = await invoke('update_provider_selection', {
        request: {
          id: 1,
          default_provider: toBackendProvider(
            selection.default_provider as any
          ),
      ...
      return {
        default_provider: toFrontendProvider(
          (updatedSelection as any).default_provider as any
        ) as any,
  ...
  async updateSMSGatewayAndroidConfig(
    config: Omit<SMSGatewayAndroidConfig, 'id' | 'created_at' | 'updated_at'>
  ): Promise<SMSGatewayAndroidConfig> {
    try {
  ...
  async testSMSGatewayAndroidConnection(
    config: Omit<
      SMSGatewayAndroidConfig,
      'id' | 'created_at' | 'updated_at'
    > & { test_phone?: string; test_message?: string }
  ): Promise<boolean> {
    try {
  ...
  async updateInfobipConfig(
    config: Omit<InfobipConfig, 'id' | 'created_at' | 'updated_at'>
  ): Promise<InfobipConfig> {
    try {
  ...
  async updateSIM800900Config(
    config: Omit<SIM800900Config, 'id' | 'created_at' | 'updated_at'>
  ): Promise<SIM800900Config> {
    try {
  ...
  async getSMSConversationMessages(
    conversationId: string
  ): Promise<SMSMessage[]> {
    try {
      return await invoke('get_sms_conversation_messages', {
