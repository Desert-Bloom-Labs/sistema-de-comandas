...
export const MODULES: Module[] = [
  ...
  },
  {
    ...
    path: '/orders',
    color: 'warning',
    description: 'Gestión de pedidos de cocina',
    names: {
      french: 'Commandes',
      english: 'Orders',
      spanish: 'Pedidos',
  ...
  },
  {
    ...
    path: '/planner',
    color: 'info',
    description: 'Planificación del local',
    names: {
  ...
  },
  {
    ...
    path: '/auditguard',
    color: 'danger',
    description: 'Detección de fraudes y cumplimiento',
    names: {
      french: 'AuditGuard',
      english: 'AuditGuard',
      spanish: 'AuditGuard',
  ...
  },
  {
    ...
    path: '/menugpt',
    color: 'purple',
    description: 'Analiza tus menús y crea productos automáticamente',
    names: {
      french: 'MenuGPT',
      english: 'MenuGPT',
      spanish: 'MenuGPT',
  ...
  },
  {
    ...
    path: '/smsticket',
    color: 'info',
    description: 'Notificaciones SMS y gestión de tickets',
    names: {
      french: 'Billet SMS',
      english: 'SMSTicket',
      spanish: 'Boleto SMS',
...
function appReducer(state: AppState, action: AppAction): AppState {
  switch (action.type) {
    case 'OPEN_MODULE': {
      ...
      const existingTabWithSameParams = state.tabs.find(tab => {
        ...
        if (action.payload.params && tab.params) {
          ...
          if (action.payload.params.selectedTable && tab.params.selectedTable) {
            return (
              action.payload.params.selectedTable.id ===
              tab.params.selectedTable.id
            );
          }
          // Pour d'autres paramètres, on peut faire une comparaison plus générale
          return (
            JSON.stringify(action.payload.params) === JSON.stringify(tab.params)
          );
        }
        ...
        return {
          ...
          tabHistory: [
            ...state.tabHistory.filter(
              id => id !== existingTabWithSameParams.id
            ),
      ...
      const existingTabsForModule = state.tabs.filter(
        tab => tab.moduleId === action.payload.moduleId
      );
    ...
    case 'CLOSE_TAB': {
      ...
      const newHistory = state.tabHistory.filter(
        id => id !== action.payload.tabId
      );
      ...
      const updatedTabs = newTabs.map(tab => {
        if (tab.moduleId === tabToClose.moduleId) {
          const moduleTabs = newTabs.filter(
            t => t.moduleId === tabToClose.moduleId
          );
    ...
    case 'CLEAR_TAB_ERROR': {
      return {
        ...state,
        tabs: state.tabs.map(tab =>
          tab.id === action.payload.tabId ? { ...tab, error: undefined } : tab
        ),
      };
    }
...
export function AppProvider({ children }: { children: ReactNode }) {
  ...
  const actions = {
    openModule: (moduleId: string, params?: any) =>
      dispatch({ type: 'OPEN_MODULE', payload: { moduleId, params } }),
